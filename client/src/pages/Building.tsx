    ...
    return(
        <div className="font-montserrat w-full min-h-screen px-4 py-6">
            <div className="flex flex-col justify-center items-center h-full max-w-2xl mx-auto space-y-5">
                <div>
                <img src="/astronaut_2.gif" className="h-[17rem]" alt="Astronaut animation" />
                </div>
                <div>
                ...
                <input type="email" value={email} onChange={handleEmail} className="rounded-l-lg border border-brand-orange text-brand-orange block flex-1 text-sm px-3.5 py-1.5 placeholder:text-brand-orange placeholder:opacity-60 focus:outline-none" placeholder="tharsis.uni@gmail.com" required />
                <button type="submit" className="flex items-center justify-center w-20 px-1 text-sm text-white bg-brand-orange font-medium rounded-r-md border border-brand-orange">
                    {
                    isLoading ? (
                        <svg className="w-5 h-5 text-gray-800 dark:text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                {
                msgSuccess ? (
                    <div className="flex px-4 py-3 mb-4 text-sm text-green-700 bg-green-100 rounded-lg space-x-2" role="alert">
                ...
                {
                msgExists ? (
                    <div className="flex px-4 py-3 mb-4 text-sm text-red-700 bg-red-100 rounded-lg space-x-2" role="alert">
                    <FontAwesomeIcon icon={faCircleXmark} size="xl" />
                    <div>
                        <span className="font-semibold">El correo ya se encuentra registrado</span>
                    </div>
                    </div>
                ) : (<></>)
                }
                ...
                <p className="text-sm font-medium text-gray-600">© {new Date().getFullYear()} Tharsis</p>
            </div>
        </div>
    )
}
